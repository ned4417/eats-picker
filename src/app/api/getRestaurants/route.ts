// app/api/getRestaurants/route.ts
...
export async function GET(request: Request) {
    ...
    const isReroll = searchParams.get('reroll') === 'true';
    ...
    try {
        ...
        if (restaurantsData.results.length === 0) {
            return NextResponse.json({ error: 'No restaurants found near the address' }, { status: 404 });
        }
